...
- AC-012: `noc migrate <project> --to feature-archive --dry-run --json` 必须零写入地报告迁移计划；真实迁移只能通过 `--apply --backup --json` 触发，并返回备份位置和 rollback 指令。
- AC-013: `noc migrate <project> --rollback <backup-id> --json` 必须校验当前项目内合法备份和 manifest，回滚前再次备份当前状态，并恢复备份中的 `noc_docs` 内容。

## Non-Goals

- CLI 不调用模型，也不自动改写业务代码。
- `work --json` 不负责执行文档更新，只负责给出计划。
- 普通 `work/index/doctor/validate/check/evidence` 命令不负责把旧 layout 静默迁移为 feature-archive。
