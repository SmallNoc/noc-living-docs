...
|---|---|---|---|---|
| 2026-07-16 | feature-archive migration CLI | `python -m unittest tests.test_feature_archive_migration -v`; `python -m unittest discover -s tests`; `python scripts/noc.py validate .`; `python scripts/release.py --check`; wheel-installed migrate smoke | PASS | 9 migration tests passed；183 total tests passed。覆盖 simplified dry-run/apply/rollback、backup required、backup manifest、invalid backup id、damaged manifest、v1 conservative legacy preservation、domain metadata、ordinary commands not migrating、unsafe paths、conflicts, and forced apply-failure restore. |
| 2026-07-16 | feature-archive evidence and impact check | `python -m unittest tests.test_feature_archive_evidence -v`; `python -m unittest tests.test_feature_archive_check -v`; `python -m unittest discover -s tests`; stage 5专项脚本 | PASS | 5 evidence tests and 6 check tests passed；174 total tests passed。覆盖 staged added/modified/deleted/renamed、diff signals、Git unavailable/non-git、record validation、脱敏、idempotency、evidence-index rebuild、damaged evidence errors、requirement/major/multi-feature check 和旧 memory-impact 合同。 |
| 2026-07-16 | feature-archive structured update | `python -m unittest tests.test_feature_archive_update -v`; `python -m unittest discover -s tests`; stage 4专项脚本 | PASS | 8 update tests passed；163 total tests passed。覆盖 patch schema、局部章节更新、稳定 id、幂等、SHA 冲突、备份、验证结果约束、旧 layout 拒绝和 wheel 安装版 update。 |
| 2026-07-16 | feature-archive candidate routing and ensure | `python -m unittest tests.test_feature_archive_routing -v`; `python -m unittest tests.test_feature_archive_ensure -v`; `python -m unittest discover -s tests` | PASS | 15 routing tests passed；8 ensure tests passed；153 total tests passed。覆盖中文 name/aliases、代码路径和测试路径证据、歧义、索引缺失只读 fallback、ensure 幂等和旧 layout 拒绝。 |
