...
|---|---|---|---|---|
| 2026-07-16 | feature-archive migration and rollback | `python -m unittest tests.test_feature_archive_migration -v`; `python -m unittest tests.test_feature_archive_migration tests.test_feature_archive_init tests.test_simplified_memory -v`; `python scripts/noc.py validate .` | PASS | 9 migration tests passed；33 focused migration/init/simplified tests passed；repository validation passed。覆盖 dry-run 零写入、apply 重建派生索引、v1 apply 后 validate、rollback SHA 恢复、冲突拒绝和普通命令不迁移。 |
