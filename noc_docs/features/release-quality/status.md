...
测试通过 Python unittest 和 pytest 运行，覆盖 CLI help、setup/init/index/check/doctor/work/feature lifecycle、Codex Skill 安装保护、release 脚本、PyPI publish workflow 和打包元数据。wheel 同时包含项目模板、带版本清单的标准 `project-living-docs` Skill 运行文件和兼容 `skills/codex` Skill 资产。当前准备版本为 `1.3.0`；本阶段不会创建 tag 或发布 PyPI。

PR 验证运行完整 tests discovery；tag 发布 workflow 在 wheel 安装后先执行 `noc setup`，再验证 simplified init，因此远程门禁覆盖 Skill 安装与新项目初始化闭环。

README 首屏现以安装、默认 feature-archive 初始化、正常使用 Codex 三步作为唯一入门流程；主要示例和生成目录对应 project memory、功能 `overview.md` 与 feature-archive 索引，v1 small/domain 保留在 Legacy v1 / Advanced compatibility。两份 Skill 的 Definition of Done 分开约束 feature-archive、simplified 和 v1 legacy，Validate workflow 通过 unittest discovery 自动覆盖全部测试文件。wheel 隔离测试覆盖完整新用户旅程、feature update、evidence、feature impact check 和迁移检查。
...
阶段 8 将 feature-archive MVP 发布准备版本定为 `1.3.0`，同步 `VERSION`、`pyproject.toml`、README 当前版本、标准 Skill manifest、CHANGELOG 和版本断言测试。wheel package list 现在包含 `noc_assets.codex_project_living_docs`，用于随包保留兼容 Skill 资产；`noc setup` 的安装目标仍是标准 `noc_assets.project_living_docs`。
...
- Python packages: `scripts`、`scripts.noclib`、`templates`、`noc_assets.project_living_docs`、`noc_assets.codex_project_living_docs`
...
- `pyproject.toml` package version 当前为 `1.3.0`，与 `VERSION`、README、CHANGELOG 和 Skill 清单保持一致。
