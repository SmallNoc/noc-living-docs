...
|---|---|---|---|---|
| 2026-07-17 | stage 8 final release preparation | `python -m unittest discover -s tests -v`; `python scripts/noc.py validate .`; `python scripts/release.py --check`; `git diff --check`; build and install isolated 1.3.0 wheel; run setup/check/init/work/ensure/update/evidence/check/migrate | PASS | 185 tests passed after one bounded rerun for a transient Windows thread-start failure；validate passed with existing `docs/` warning；release check passed for 1.3.0；diff check had only CRLF warnings；`WHEEL_STAGE8_OK noc_living_docs-1.3.0-py3-none-any.whl` with 33 wheel entries checked, setup/check `ready`, feature impact check `passed`, migrate dry-run `dry_run`, apply `applied`, rollback `rolled_back`. |
